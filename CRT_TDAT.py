...
from utils.crt_dataloader import TriComprehensiveRobustnessDataset
from attacks.customized_pgd import EarlyStopPGDAttack

def get_args_parser():
    parser = argparse.ArgumentParser(description='Trains a CIFAR Classifier')
    parser.add_argument('--gpu', default='0')
    parser.add_argument('--num_workers', '--cpus', default=64, type=int)
    # dataset:
    parser.add_argument('--dataset', '--ds', default='IN', choices=['IN'], help='which dataset to use')
    parser.add_argument('--data_root_path', '--drp', default='/ssd1/haotao/datasets', help='dataset root path.')
    parser.add_argument('--model', '--md', default='nf_resnet50', choices=['nf_resnet26', 'nf_resnet50', 'resnet26', 'resnet50'], help='which model to use')
    # Optimization options
    parser.add_argument('--epochs', '-e', type=int, default=30, help='Number of epochs to train.')
    parser.add_argument('--decay_epochs', '--de', default=[10,20], nargs='+', type=int, help='milestones for multisteps lr decay')
    parser.add_argument('--opt', default='sgd', choices=['sgd', 'adam'], help='which optimizer to use')
    parser.add_argument('--decay', default='cos', choices=['cos', 'multisteps'], help='which lr decay method to use')
    parser.add_argument('--lr', type=float, default=0.1, help='Initial learning rate.')
    parser.add_argument('--batch_size', '-b', type=int, default=255, help='Batch size.')
    ...
    # attack
    parser.add_argument('--attacker', default='PGD', choices=['PGD'], help='which model to use')
    parser.add_argument('--steps', default=20, type=int, help='attack iteration number')
    parser.add_argument('--eps', default=8, type=int, help='eps for pgd attack')
    parser.add_argument('--alpha', default=-1, type=int, help='attack step size in uint8; -1 means auto step size.')
...
def train(gpu_id, ngpus_per_node, args):
    ...
    # train_batch_size = int(train_batch_size/2) # NOTE: Since we have two sub-batches: one for clean images and the other for DeepAug images. 
    train_batch_size = int(train_batch_size/3) # NOTE: Since we have three sub-batches: one for clean images, one for DeepAug images, and one for TDA images.
    # For example, if args.batch_size = 512, then there are 256 for each sub-batch.
    # The lr is set according to the effective batch size args.batch_size.
    ...
    train_data, val_data = imagenet_datasets(data_dir=os.path.join(args.data_root_path, 'imagenet'), num_classes=num_classes)
    edsr_data = imagenet_deepaug_dataset(data_dir=os.path.join(args.data_root_path, 'imagenet-DeepAug-EDSR'), num_classes=num_classes)
    cae_data = imagenet_deepaug_dataset(data_dir=os.path.join(args.data_root_path, 'imagenet-DeepAug-CAE'), num_classes=num_classes)
    texture_debias_data = imagenet_texture_debias_dataset(data_dir=os.path.join(args.data_root_path, 'imagenet'), num_classes=num_classes)
    # combine datasets:
    ...
    # attacker:
    attacker = PGDAttack(NormalizedModel(model, mean, std), eps=args.eps/255., nb_iter=args.steps, eps_iter=args.alpha/255., targeted=False) 
    attacker_val = PGDAttack(NormalizedModel(model, mean, std), eps=args.eps/255., nb_iter=args.steps, eps_iter=args.alpha/255., targeted=False) 
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, ((images, labels), (images_deepaug, labels_deepaug), (images_texture_debias, labels_texture_debias)) in enumerate(train_loader):
            ...
            images_deepaug = images_deepaug.to(device)
            labels_deepaug = labels_deepaug.to(device)
            images_texture_debias = images_texture_debias.to(device)
            labels_texture_debias = labels_texture_debias.to(device)

            # logits for clean imgs:
            logits = model(images)
            logits_deepaug = model(images_deepaug)
            logits_texture_debias = model(images_texture_debias)

            # loss:
            loss_clean = loss_fn(logits, labels)
            loss_deepaug = loss_fn(logits_deepaug, labels_deepaug)
            loss_texture_debias = loss_fn(logits_texture_debias, labels_texture_debias)
            ...
            if args.Lambda != 0 :
                ...
                loss = (loss_clean + loss_deepaug + loss_texture_debias + loss_adv)/4
            
            else:
                loss = (loss_clean + loss_deepaug + loss_texture_debias)/3
        ...
        if rank == 0:
            ...
            eval_this_epoch = (epoch % 7 == 0) or (epoch>=int(0.8*args.epochs)) # boolean
            ...
            for i, (images, labels) in enumerate(val_loader):
                ...
                if args.Lambda != 0 and eval_this_epoch:
                    with ctx_noparamgrad_and_eval(model):
                        images = images * std + mean # range back to [0,1]
                        images_adv = attacker_val.perturb(images, labels)
                        images_adv = (images_adv-mean)/std
                    logits_adv = model(images_adv)
            ...
            if args.Lambda != 0:
                if eval_this_epoch:
                    val_str = 'Epoch %d | Validation | Time: %.4f | lr: %s | SA: %.4f | RA: %.4f' % (
                        epoch, (time.time()-start_time), current_lr, val_SA[-1], val_RA[-1])
                else:
            ...
            if epoch == int(args.epochs/3)-1 or epoch == int(args.epochs/3*2)-1:
                save_ckpt_adv(epoch, model, optimizer, scheduler, best_SA, best_RA, training_loss, val_SA, val_RA,
                    os.path.join(save_folder, 'epoch%d.pth' % epoch))
