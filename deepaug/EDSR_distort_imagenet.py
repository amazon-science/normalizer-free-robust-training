...
parser.add_argument('--total_workers', default=None, type=int, required=True)
parser.add_argument('--worker_number', default=None, type=int, required=True) # MUST BE 0-INDEXED
parser.add_argument('--data_dir', '--dd', default='/home/haotao/datasets/imagenet', help='ImageNet dataset path.')
parser.add_argument('--save_root_path', '--srp', default='/ssd1/haotao/datasets', help='ImageNet dataset path.')
...
class FolderWithPath(ImageNetSubsetDataset):
    ...
    def __getitem__(self, index):
        ...
        save_path = os.path.join(args.save_root_path, 'imagenet-DeepAug-EDSR/train/', self.idx_to_class[target])
...
distorted_dataset = FolderWithPath(root=os.path.join(args.data_dir, 'train'), transform=test_transform)

distorted_dataset[0]

loader = torch.utils.data.DataLoader(distorted_dataset, batch_size=16, shuffle=True)
